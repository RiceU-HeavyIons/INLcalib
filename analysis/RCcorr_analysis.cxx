...
int main(int argc, char *argv[])
{
  ...
  for(int i=0;i<8;i++){
    ...
    tdigname2[i]="tdig-ser"+tdigname[i];
  ...
  for(int iboard=0;iboard<8;iboard++){
    ...
    for(int itdc=0;itdc<3;itdc++){
      for(int ich=0;ich<8;ich++){
        ...
        sprintf(tmpchr,"../result/%s/inl/%s.tdc%d.inl",tdigname2[iboard].c_str(),tdigname2[iboard].c_str(),itdc);
        if(processmode=="8")sprintf(tmpchr,"../result/%s/inl/%s.tdc%d.ch%d.inl",tdigname2[iboard].c_str(),tdigname2[iboard].c_str(),itdc,ich);
