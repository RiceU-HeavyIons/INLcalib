...
int main(int argc, char *argv[])
{
    ...
    for(int itdig=0;itdig<3;itdig++){
      for(int ich=0;ich<8;ich++){
        for(int i=0;i<1024;i++){
          int mm=(i/2)*2;
          int nn=mm+1;
          inlcor2[iboard][itdig][ich][i]=(inlcor[iboard][itdig][ich][mm]+inlcor[iboard][itdig][ich][nn])/2.;
        }
      }
    }
  }
  ...
  while (numret=fread(&nwords,4,1,inputfile)==1) {
    ...
    for(int iword=0;iword<nwords;iword++){
      ...
      if((edgeid != 4)){
	if(wrongedge<50){
	  //cout<<"ERROR!!Wrong edge="<<edgeid<<hex<<"0x"<<tempv<<endl;
	}
	wrongedge++;
	continue;
      } 
      ...
      //time[tdigboardid][tdcid][tdcchan]=itime+int(0.5+inlcor[tdigboardid][tdcid][tdcchan][bin]);
      time[tdigboardid][tdcid][tdcchan]=itime+int(100*inlcor2[tdigboardid][tdcid][tdcchan][bin])/100.;
    ...
    for(int itdc=0;itdc<3;itdc++){	 
      for(int i=0;i<8;i++){
        for(int j=0;j<8;j++){
          ...
          int  maxbin=deltatimeh[iboard][itdc][i][j]->GetMaximumBin();
          mean = deltatimeh[iboard][itdc][i][j]->GetBinCenter(maxbin);
      ...
      sprintf(tmpchr,"../result/%s/root/%s.tdc%d.corr1.root",tdigname[iboard].c_str(),tdigname[iboard].c_str(),itdc);
      if(processmode=="8")sprintf(tmpchr,"../result/%s/root/%s.tdc%d.corr8.root",tdigname[iboard].c_str(),tdigname[iboard].c_str(),itdc);
    ...
    for(int itdc=0;itdc<3;itdc++){
      ...
      for(int i=0;i<8;i++){
        for(int j=0;j<8;j++){
          ...
          ipage=ifig/4;
          if(lastpage !=ipage) {
            sprintf(tmpchr,"../result/%s/gif/%s-tdc%d-page%d-deltatime.gif",tdigname[iboard].c_str(),tdigname[iboard].c_str(),itdc,ipage);
            c1->Print(tmpchr);
            lastpage=ipage;
          }
        }
