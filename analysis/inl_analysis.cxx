...
int main(int argc, char *argv[])
{
  ...
  for(int iboard=0;iboard<8;iboard++){
    ...
    float lsbdata[2048];      
    float lsbdata1024[1024];//dmac
    float dnl2048[2048];
    float inl2048[2048];
    for(int i=0;i<2048;i++){lsbdata[i]=0;dnl2048[i]=0;inl2048[i]=0;}
    for(int i=0;i<1024;i++){lsbdata1024[i]=0;}//dmac
   ...
   for(int itdc=0;itdc<3;itdc++){
	...
	//dmac
	for(int ibin=0;ibin<2048;ibin++){
	  float lsbval = lsbdata[ibin];
	  if(ibin<1024){
	    lsbdata1024[ibin]=lsbval;
	  }
	  else{
	    lsbdata1024[ibin-1024]=lsbval+lsbdata1024[ibin-1024];
	  }
	}
...
//         }
	...
	for(int ibin=0;ibin<1024;ibin++){
          TotHit=TotHit+lsbdata1024[ibin];
          TotBin++;
        }
...
//         }
	for(int ibin=0;ibin<TotBin;ibin++){
          float dnl=(lsbdata1024[ibin] - expectedhit )/expectedhit;
          interdnl=interdnl+dnl;
          int tempinl=int(interdnl*10000.);
          inlcor[ibin]=tempinl/10000.;
          dnl1024h[iboard][itdc][ich]->SetBinContent(ibin,dnl);
        ...
        // write out INL table
        sprintf(tmpchr,"../result/%s/inl/%s.tdc%d.ch%d.inl",tdigname[iboard].c_str(),tdigname[iboard].c_str(),itdc,ich);
        if(processmode=="1")sprintf(tmpchr,"../result/%s/inl/%s.tdc%d.inl",tdigname[iboard].c_str(),tdigname[iboard].c_str(),itdc);
    ...
    sprintf(tmpchr,"../result/%s/root/%s.inl.root",tdigname[iboard].c_str(),tdigname[iboard].c_str());
...
//=====================================================
  ...
  for(int iboard=0;iboard<8;iboard++){
    ...
    //c1->Modified();
    //gtitle1->Draw();
    sprintf(tmpchr,"../result/%s/gif/%s-inl.gif",tdigname[iboard].c_str(),tdigname[iboard].c_str());
    if(processmode=="1")sprintf(tmpchr,"../result/%s/gif/%s-inl1.gif",tdigname[iboard].c_str(),tdigname[iboard].c_str());
    c1->Print(tmpchr); 

    c2->cd();
    checkchannel2h[iboard]->SetMarkerSize(2);    
    checkchannel2h[iboard]->Draw("text box col");    
    sprintf(tmpchr,"../result/%s/gif/%s-checkchannel.gif",tdigname[iboard].c_str(),tdigname[iboard].c_str());
    c2->Print(tmpchr); 
